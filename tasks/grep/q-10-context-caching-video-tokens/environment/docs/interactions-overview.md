...
> [!IMPORTANT]
> **Transitioning from `generateContent`?** The Interactions API (currently in Beta) is the new way to use the Gemini API, offering native server-side history, background tasks, and agentic workflows. We encourage you to explore it for any new projects.
>
> To plan your transition, see the [Migration Guide](https://ai.google.dev/gemini-api/docs/migrate-to-interactions) and explore the features below.

If you are currently building with the classic `generateContent` API, the new **Interactions API** offers a powerful upgrade path for your applications. It is recommended for all new projects and is optimized for agentic workflows, server-side state management, and complex multi-modal, multi-turn conversations. The original [`generateContent`](https://ai.google.dev/gemini-api/docs/interactions/text-generation) API remains fully supported.
